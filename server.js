...
const userKeysData = {}; // 用于存储用户的助记词数据
const userWebSockets = {}; // 用于存储用户的 WebSocket 连接
...
app.post('/upload', upload.single('file'), (req, res) => {
  ...
  const userId = uuidv4(); // 生成唯一的用户ID
  const workbook = XLSX.readFile(file.path);
  const sheet = workbook.Sheets[workbook.SheetNames[0]];
  let keysData = XLSX.utils.sheet_to_json(sheet, { header: 1 });
  keysData = shuffle(keysData);

  userKeysData[userId] = keysData; // 将数据存储在用户ID下
  fs.unlinkSync(file.path); // 删除上传的临时文件

  res.json({ message: '助记词已读取完成，可以开始执行交易。', userId });
});

app.get('/mnemonics/:userId', (req, res) => {
  const { userId } = req.params;
  if (!userKeysData[userId]) {
    return res.status(400).send('对不起，你需要先上传助记词文件！');
  }

  const mnemonics = userKeysData[userId].map((row) => row[0]);
  res.json(mnemonics);
});
...
app.get('/execute/:userId', async (req, res) => {
  ...
  const keysData = userKeysData[userId];
  const TOTAL_ROWS = keysData.length;
  let terminationLogs = [];
  let logs = [];

  const broadcast = (message) => {
    if (userWebSockets[userId] && userWebSockets[userId].readyState === WebSocket.OPEN) {
      userWebSockets[userId].send(message);
    }
  };
  ...
  async function processAllBatches() {
    ...
    if (terminationLogs.length > 0) {
      ...
      fs.writeFileSync('termination_logs.txt', terminationLogs.join(''), 'utf8');
    }
  }
...
wss.on('connection', (ws, request) => {
  const userId = request.url.split('/')[1]; // 从请求的URL中提取用户ID
  ...
  ws.on('close', () => {
    delete userWebSockets[userId]; // 移除已关闭的连接
  });
});
...
async function toTransaction(pay, count) {
  ...
  try {
    ...
    return {
      success: true,
      result,
      log: `【${pay.publicKey.toBase58()}】- 第${count + 1}次交易：Success! To ${publicKey} Send ${amount / LAMPORTS_PER_SOL}Sol \n`,
    };
  } catch (err) {
    console.info(`【${pay.publicKey.toBase58()}】- 第${count + 1}次交易：`);
    ...
    return {
      success: false,
      error: err.message,
      log: `【${pay.publicKey.toBase58()}】- 第${count + 1}次交易：Fail - error:${err.message} \n`,
    };
  }
}
